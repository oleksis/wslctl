...
task Test {
    try {
        Write-Verbose -Message "Running PSScriptAnalyzer on wslctl.ps1"
        Invoke-ScriptAnalyzer ".\wslctl.ps1"
    }
    catch {
...
task DebugBuild -if ($Configuration -eq "debug") {
    ...
    if(!($ModuleVersion)) {
        Write-Verbose -Message "No new ModuleVersion was provided, locating existing version from ps1 file."
        ...
        Write-Verbose "ModuleVersion found from ps1 file: $ModuleVersion"
    }
